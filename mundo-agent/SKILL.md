...
| `references/mundo-module-class-map.md` | **v2.0.9新增** — 蒙多模块类名映射（测试/导入必备） |
...
> 详细数据：`references/performance-benchmark-v2.0.9.md`
...
**批量更新命令（当版本不一致时）：**
```bash
cd ~/.hermes/mundo-agent && sed -i '' 's/v2\.0\.8/v2.0.9/g' tools.py policy.py sandbox.py events.py memory.py approval.py model_adapter.py agents.py
```
