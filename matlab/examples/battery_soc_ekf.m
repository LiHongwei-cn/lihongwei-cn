...
%% EKF 估计
% 状态向量 x = [SOC; V1]
% 观测: V_term = OCV(SOC) - R0*I - V1
soc_ekf = zeros(1, n);
soc_ekf(1) = 0.88;  % 相同初始误差
...
for k = 2:n
    ...
    % 观测雅可比 (dOCV/dSOC = 0.9 + 0.2*SOC - 0.15*SOC^2)
...
grid on; title('Battery Terminal Voltage');
...
legend('True', 'Coulomb Counting', 'EKF', 'Location', 'best');
grid on; title('SOC Estimation Comparison');
