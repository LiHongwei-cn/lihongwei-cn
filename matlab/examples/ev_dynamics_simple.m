...
ylabel('Motor Torque (Nm)'); grid on;
title('Motor Torque Output');

subplot(3,2,3);
plot(t, s/1000, 'b-', 'LineWidth', 1.5);
ylabel('Distance (km)'); grid on;
title('Cumulative Distance');

subplot(3,2,4);
plot(t, P_m/1000, 'r-', 'LineWidth', 1.5);
ylabel('Motor Power (kW)'); grid on;
title('Motor Power');

subplot(3,2,5);
plot(t, E_cons, 'm-', 'LineWidth', 1.5);
xlabel('Time (s)'); ylabel('Cumulative Energy (Wh)'); grid on;
