...
legend('Reference', 'Actual'); grid on; title('Speed Response');

subplot(3,2,2);
plot(t, Te_log, 'b-', 'LineWidth', 1.2);
xlabel('Time (s)'); ylabel('Electromagnetic Torque (Nm)');
grid on; title('Torque Output');

subplot(3,2,3);
plot(t, id_log, 'r-', t, iq_log, 'b-', 'LineWidth', 1.2);
xlabel('Time (s)'); ylabel('Current (A)');
legend('i_d', 'i_q'); grid on; title('d-q Axis Current');

subplot(3,2,4);
plot(t, vd_log, 'r-', t, vq_log, 'b-', 'LineWidth', 1.2);
xlabel('Time (s)'); ylabel('Voltage (V)');
legend('v_d', 'v_q'); grid on; title('d-q Axis Voltage');

subplot(3,2,5:6);
plot(t, w_ref, 'r--', t, wm_log, 'b-', 'LineWidth', 1.2);
xlim([0.38 0.5]);
xlabel('Time (s)'); ylabel('Speed (rpm)');
legend('Reference', 'Actual'); grid on; title('Load Response (Zoom)');

%% 性能指标
fprintf('===== Motor Control Simulation Results =====\n');
fprintf('Steady-state speed error: %.2f rpm\n', abs(wm_log(end) - w_ref(end)));
fprintf('Peak torque: %.3f Nm\n', max(Te_log));
fprintf('q-axis steady current: %.3f A\n', iq_log(end));
