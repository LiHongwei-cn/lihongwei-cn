...
title('Speed Closed-Loop Response');

subplot(3,2,2);
plot(t, w_log, 'b-', 'LineWidth', 1.2);
xlim([0.75 0.9]);
xlabel('Time (s)'); ylabel('Speed (rpm)');
grid on; title('Acceleration Phase (Zoom)');
...
legend('Load Torque', 'Electromagnetic Torque'); grid on;
...
%% 结果
fprintf('===== DC Motor Control Simulation =====\n');
fprintf('Steady-state speed error: %.1f rpm\n', abs(w_log(end) - w_ref(end)*30/pi));
fprintf('Peak armature current: %.2f A\n', max(abs(ia_log)));
fprintf('Settling time: ~0.15 s (to +/-5%% band)\n');
