...
for i = 1:n
    ...
    P_dem(i) = max(P_wheel, -P_mot_max);  % 回馈功率有上限
end
...
for i = 2:n
    ...
    if soc < SOC_min
        P_gen1(i) = P_gen_max;  % 开启增程器
    elseif soc > SOC_max
        P_gen1(i) = 0;          % 关闭增程器
    else
        P_gen1(i) = P_gen1(i-1); % 保持上一状态
    end
...
for i = 2:n
    ...
    % 功率跟随：增程器输出跟踪需求功率 + SOC 修正
    if soc < SOC_center - SOC_band
        P_set = P_d + 0.3 * P_gen_max;  % SOC 低，多发电
    elseif soc > SOC_center + SOC_band
        P_set = max(0, P_d * 0.5);      % SOC 高，少发电
    else
        P_set = P_d;                     % 跟踪需求
    end

    % 增程器限制（最小运行功率 15kW，低于此值关机）
...
%% ===== 结果对比 =====
fuel_rate = 0.23;  % 增程器燃油消耗率 [L/kWh]
fuel1 = sum(P_gen1) * dt / 3600 / 1000 * fuel_rate;  % [L]
fuel2 = sum(P_gen2) * dt / 3600 / 1000 * fuel_rate;

dist = sum(v_ms * dt) / 1000;  % [km]

fprintf('===== Energy Management Strategy Comparison =====\n');
fprintf('Distance: %.1f km\n', dist);
fprintf('\n%-18s %-15s %-15s\n', 'Metric', 'Thermostat', 'Power Follow');
fprintf('%-18s %-15.1f %-15.1f\n', 'Fuel/100km (L)', fuel1/dist*100, fuel2/dist*100);
fprintf('%-18s %-15.2f %-15.2f\n', 'Final SOC', SOC1(end), SOC2(end));
fprintf('%-18s %-15.2f %-15.2f\n', 'SOC Std Dev', std(SOC1), std(SOC2));
fprintf('%-18s %-15d %-15d\n', 'On/Off Count', ...
...
grid on; title('Drive Demand Power');
...
ylabel('RE Power (kW)');
xlabel('Time (min)'); grid on;
title('Strategy 2: Power Following');

subplot(4,2,5);
plot(t/60, cumsum(P_gen1)*dt/3600/1000, 'r-', t/60, cumsum(P_gen2)*dt/3600/1000, 'b-', 'LineWidth', 1.2);
xlabel('Time (min)'); ylabel('Cumulative Gen (kWh)');
legend('Thermostat', 'Power Following'); grid on;
...
xlabel('Demand Power (kW)'); ylabel('RE Power (kW)');
grid on; legend('Thermostat', 'Power Following');
title('Power Split Scatter');

subplot(4,2,7:8);
bar_data = [fuel1, fuel2; fuel1/dist*100, fuel2/dist*100]';
bar(bar_data); set(gca, 'XTickLabel', {'Thermostat', 'Power Foll.'});
legend('Total Fuel (L)', 'Fuel per 100km (L/100km)', 'Location', 'northwest');
